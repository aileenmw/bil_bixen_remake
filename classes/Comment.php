...
//    }    
    ...
    function messageCount($mysqli, $car_id) {
        ...
        $sql = "SELECT  Count(`message`) FROM `bil_bixen_kommentarer`
                    ...
                    WHERE biler_id = $car_id AND `status` = 2";
    ...
    function createMessage($mysqli, $comment, $name, $email, $car_id, $status) {
    $comment = $mysqli->real_escape_string($comment);
    $name = $mysqli->real_escape_string($name);
    $email = $mysqli->real_escape_string($email);
    $car_id = $mysqli->real_escape_string($car_id);

    $sql = "INSERT INTO `bil_bixen_kommentarer`( `message`, `name`, `email`, `biler_id`,`staus`) 
            VALUES ( '$comment', '$name', '$email', $car_id, $status)";
    ...
    if (empty($comment) or empty($name) or empty($email)) {
        ...
        if ($result) {
              $message = 'Dine data er :' . '<br><br>Kommentar: ' . $comment . '<br>Navn :' . $name . '<br>'
            . 'Email: ' . $email . '<br>';
        }
    }
}
